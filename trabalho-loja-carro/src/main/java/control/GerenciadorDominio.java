...
public class GerenciadorDominio {
    ...
    public List<Veiculo> pesquisarVeiculo(String pesq, int tipo) throws HibernateException {
        ...
        switch (tipo) {
            case 0:
                return veiDAO.pesquisarPorMarca(pesq);
            case 1:
                return veiDAO.pesquisarPorModelo(pesq);
            case 2:
                return veiDAO.pesquisarPorPlaca(pesq);
            default:
                return null;
        }
    ...
    public List<Venda> pesquisarVenda(String pesq, int tipo) throws HibernateException {
        ...
        switch (tipo) {
            case 0:
                return venDAO.pesquisarPorMarca(pesq);
            case 1:
                return venDAO.pesquisarPorModelo(pesq);
            case 2:
                return venDAO.pesquisarPorPlaca(pesq);
            default:
                return null;
        }
    ...
    public List<Compra> pesquisarCompra(String pesq, int tipo) throws HibernateException {
        ...
        switch (tipo) {
            case 0:
                return comDAO.pesquisarPorMarca(pesq);
            case 1:
                return comDAO.pesquisarPorModelo(pesq);
            case 2:
                return comDAO.pesquisarPorPlaca(pesq);
            default:
                return null;
        }
    ...
    public void excluir(Object obj) throws SQLException, ClassNotFoundException, HibernateException {

        if (obj instanceof Pessoa) {
            Pessoa pessoa = (Pessoa) obj;
            // Verifica se a pessoa está relacionada a alguma compra
            if (!pessoa.getCompras().isEmpty()) {
                throw new HibernateException("Esta pessoa esta vinculada a uma compra!");
            }
        }else if (obj instanceof Veiculo){
            Veiculo veiculo = (Veiculo) obj;
            // Verifica se a veiculo está relacionado a alguma compra
            if (!veiculo.getCompras().isEmpty()) {
                throw new HibernateException("Este veiculo esta vinculado a uma compra!");
            }
        }
