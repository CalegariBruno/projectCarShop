...
public class CompraAbstractTableModel extends AbstractTableModel {
    ...
    public Object getValueAt(int rowIndex, int columnIndex) {
        ...
        switch (columnIndex) {
            case 0:
                return item.getRevendedor().getNome();
            case 1:
                return item.getVeiculo().getTipo();
            case 2:
                return item.getVeiculo().getPlaca();
            case 3:
                return item.getVeiculo().getRenavam();
            case 4:
                return item.getVeiculo().getMarca();
            case 5:
                return item.getVeiculo().getModelo();
            case 6:
                return item.getVeiculo().getAno();
            case 7:
                return item.getVeiculo().getCor();
            case 8:
                return item.getVeiculo().getCombustivel();
            case 9:
                return item.getData();
            case 10:
